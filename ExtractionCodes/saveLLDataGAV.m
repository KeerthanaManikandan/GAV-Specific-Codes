...
function LLFileExistsFlag = saveLLDataGAV(monkeyName,expDate,protocolName,folderSourceString,gridType,type)

FsEye=200;
eyeRangeMS{1} = [-320 320]; timePeriodMS{1} = [-500   500]; maxStimPos{1} = 20;
eyeRangeMS{2} = [-480 800]; timePeriodMS{2} = [-1000 1000]; maxStimPos{2} = 10;

% folderName    = [folderSourceString 'Data\rawData\' monkeyName expDate '\'];
...
[~,folderName]=getFolderDetails(dataLog); % MD
...
LLFileExistsFlag = 0;
    ...
    elseif strncmpi(protocolName,'GAV',3) % For GAV Protocol, added by MD 16-03-2015
        LL = getStimResultsLLGAV(monkeyName,gridType,expDate,protocolName,folderSourceString,1,0);%,eyeRangeMS{type},FsEye);
    %     saveEyeDataInDegGAV(monkeyName,expDate,protocolName,folderSourceString,gridType,timePeriodMS{type},FsEye,maxStimPos{type});
        LLFileExistsFlag = 1;
