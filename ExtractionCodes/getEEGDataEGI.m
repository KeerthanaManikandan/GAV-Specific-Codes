...
[EEGData,Fs] = loadEGIFile(folderIn,fileName);

cAnalog = size(EEGData,1);
Fs = Fs/decimationFactor; % added by MD; default is 1
...
if (cAnalog>0)
    ...
    if decimateFlag
        ...
        for iD = 1:cAnalog
           EEGData(iD,:) = decimate(EEGData(iD,:),decimationFactor);
        end
    end
    
    times = 0:1/Fs:(size(EEGData,2)-1)/Fs;
    ...
    if reRefFlag
        ...
        refData = EEGData(refElec,:);
        for iR = 1:cAnalog
            EEGData(iR,:) = EEGData(iR,:) - refData;
        end
        EEGData(refElec,:) = (-1).*refData;
    end
    
    % Notch filters [MD]; taken from Vinay's method in blackrock extraction
    if notchLineNoise
        for iN = 1:cAnalog
            N = length(EEGData(iN,:)); % length of the full length analog data
            ...
            analogDataFullfft = fft(EEGData(iN,:));
...
%         disp(['elec' num2str(analogInputNums(i))]);
        ...
        for j=1:totalStim
            analogData(j,:) = EEGData(analogInputNums(i),goodStimPos(j)+1:goodStimPos(j)+numSamples);
        end
...
%         save(fullfile(outputFolder,['elec' num2str(analogInputNums(i)) '.mat']),'analogData','analogInfo');
        save(fullfile(outputFolder,['elec' num2str(analogInputNums(i)) '.mat']),'analogData');
    end
    close(hW);
...
function [EEGData,EEGSamplingRate] = loadEGIFile(folderIn,fileName)
