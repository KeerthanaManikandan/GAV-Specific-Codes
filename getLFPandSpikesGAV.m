...
function [electrodeNums,elecSampleRate,AinpSampleRate] = getLFPandSpikesGAV(fileName,analogElectrodesToStore,folderIn,folderOut,...
...
electrodeCount = 0;
electrodeNums = []; % added by MD 28-10-14 for noe electrode condition
ainpCount = 0;
...
if getLFP && (cAnalog>0)
    ...
    if electrodeCount ~= 0
        if isempty(analogElectrodesToStore)
            disp('Taking all available electrodes.');
